...
let scores = document.querySelector('#score-display'); // Inserindo pontuação ao jogo
let counter = 0;
let alienInterval;
    ...
    let laserInterval = setInterval(() => {
        ...
        aliens.forEach((alien) => { //comparando se cada alien foi atingido, se sim, troca o src da imagem
            if(checkLaserCollision(laser, alien)) {
                ...
                scores.innerHTML = counter++; // Inserindo pontuação ao jogo
...
function gameOver() {
    ...
    setTimeout(() => {
        alert('GAME OVER!\nSua pontuação foi ' + (counter-1) + " ponto(s)");
        yourShip.style.top = "250px";
        startButton.style.display = "block";
        instructionsText.style.display = "block";
        counter = 0; // reiniciando pontuação ao jogo
        scores.innerHTML = counter; // reiniciando pontuação ao jogo
    });
}